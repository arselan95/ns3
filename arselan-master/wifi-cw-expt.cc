...
double experiment (std::string wifiManager, double cwMin, double cwMax)
{
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
     ...
     tput += i->second.rxBytes * 8.0 / (i->second.timeLastRxPacket.GetSeconds() - i->second.timeFirstTxPacket.GetSeconds()) / 1000 / 1000;
        }
    }
...
int main (int argc, char **argv)
{
  ...
  cmd.AddValue ("cwMin", "Set cwMin", cwMin);
  cmd.AddValue ("cwMax", "set cwMax", cwMax); 
  cmd.Parse (argc, argv);

//added
std::ofstream plotFile ("wifi-cwExpt-output.plt");
  GnuplotCollection gnuplots ("wifi-cwExpt.pdf");
  Gnuplot plot;
  plot.AppendExtra ("set xlabel 'cwMin'");
  plot.AppendExtra ("set ylabel 'Throughput (Mbps)'");
  plot.AppendExtra ("set key outside");

  for(uint32_t cwExpMax = cwMax; cwExpMax <= 256; cwExpMax*=2){
   // double cwMax = pow( 2.0, (double)cwExpMax);
    Gnuplot2dDataset dataset;
    dataset.SetStyle (Gnuplot2dDataset::LINES);
    for(uint32_t cwExpMin = cwMin; cwExpMin <= 64; cwExpMin*=2){
      //  double cwMin = pow( 2.0, (double)cwExpMin);
        double avgTput = experiment(wifiManager,(double)cwExpMin,(double) cwExpMax);
        dataset.Add(cwExpMin, avgTput);
